...
module.exports = class extends Generator {
  constructor(args, opts) {
    ...
    this.option("insiders", {
      type: Boolean,
      alias: "i",
      description: "Show the insiders options for the generator",
    });
    this.option("quick", {
      type: Boolean,
      alias: "q",
    ...
    this.option("open", {
      type: Boolean,
      alias: "o",
      description: "Open the generated extension in Visual Studio Code",
    });
    this.option("openInInsiders", {
      type: Boolean,
      alias: "O",
      description:
    ...
    this.option("webpack", {
      type: Boolean,
      description: `Bundle the extension with webpack`,
    });
    this.option("gitInit", {
      type: Boolean,
    ...
    this.option("snippetFolder", {
      type: String,
      description: `Snippet folder location`,
    });
    this.option("snippetLanguage", {
      type: String,
  ...
  async initializing() {
    ...
    if (!this.extensionConfig.insiders) {
      ...
      this.log(
        yosay("Welcome to the Visual Studio Code Insiders Extension generator!")
      );
    }
  ...
  async prompting() {
    ...
    if (extensionType) {
      ...
      const extensionGenerator = extensionGenerators.find(
        (g) => g.aliases.indexOf(extensionType) !== -1
      );
      if (extensionGenerator) {
        ...
        this.log(
          "Invalid extension type: " +
            extensionType +
            "\nPossible types are: " +
            extensionGenerators.map((g) => g.aliases.join(", ")).join(", ")
        );
        this.abort = true;
      }
    } else {
  ...
  writing() {
    ...
    this.sourceRoot(
      path.join(__dirname, "./templates/" + this.extensionConfig.type)
    );
  ...
  async end() {
    ...
    if (this.extensionConfig.proposedAPI) {
      this.spawnCommand(this.extensionConfig.pkgManager, [
        "run",
        "update-proposed-api",
      ]);
    }
    ...
    this.log(
      "Your extension " + this.extensionConfig.name + " has been created!"
    );
    this.log("");

    const [codeStableLocation, codeInsidersLocation] = await Promise.all([
      witch("code").catch(() => undefined),
      witch("code-insiders").catch(() => undefined),
    ]);

    if (
      !this.extensionConfig.insiders &&
      !this.options["open"] &&
      !this.options["openInInsiders"] &&
      !this.options["quick"]
    ) {
      const cdLocation =
        this.options["destination"] || this.extensionConfig.name;
    ...
    if (this.options["open"]) {
      ...
      if (this.options["quick"]) {
        this.spawnCommand(codeInsidersLocation || codeStableLocation, [
