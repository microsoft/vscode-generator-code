...
connection.onCodeAction((params) => {
	...
	return [CodeAction.create(title, Command.create(title, '<%= name %>.fixMe', textDocument.uri), CodeActionKind.QuickFix)];
});

connection.onExecuteCommand(async (params) => {
	if (params.command !== '<%= name %>.fixMe' || params.arguments ===  undefined) {
		return;
	}
