...
describe('test code generator', function () {
	...
	it('command-ts with webpack + bun', async () => {
		this.timeout(10000);
	...
	it('language pack with bun', async () => {
		const runResult = await helpers.run(appLocation).withAnswers({
			type: 'ext-localization',
			name: "vscode-language-pack-ru",
			displayName: "Russian Language Pack",
		...
		try {
			const expectedPackageJSON = {
				...
				"scripts": {
					"update": "cd ../vscode && bun run update-localization-extension ru"
				}
			};
	...
	it('command-web with bun', async () => {
		...
		try {
			const expectedPackageJSON = {
				...
				"scripts": {
					"test": "vscode-test-web --browserType=chromium --extensionDevelopmentPath=. --extensionTestsPath=dist/web/test/suite/index.js",
					"pretest": "bun run compile-web",
					"vscode:prepublish": "bun run package-web",
					"compile-web": "webpack",
					"watch-web": "webpack --watch",
					"package-web": "webpack --mode production --devtool hidden-source-map",
					"lint": "eslint src --ext ts",
	...
	it('command-web-esbuild with yarn', async () => {
		...
		try {
			...
			assertFiles(runResult, 'testCom', ['src/web/extension.ts', 'esbuild.js', 'src/web/test/suite/extension.test.ts', 'src/web/test/suite/mochaTestRunner.ts', 'tsconfig.json']);
...
it('sample notebook renderer with bun', async () => {
	...
	try {
		...
		const expectedPackageJSON = {
			...
			"scripts": {
				...
				"lint": "eslint src --ext ts",
				"watch": "webpack --mode development --watch",
				"pretest": "webpack --mode development && bun run lint",
				"test": "vscode-test"
			},
			"devDependencies": devDependencies([
				"@types/mocha",
